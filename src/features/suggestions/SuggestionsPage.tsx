...
export function SuggestionsPage() {
  ...
  const settleMutation = useMutation({
    mutationFn: () => suggestionService.settlePending(),
    onSuccess: (data) => {
      toast.success(
        `Liquidação: ${data.settled} sugestões (${data.won} ganhas, ${data.lost} perdidas, ${data.voided} void)`,
      );
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="grid gap-2 text-sm text-muted-foreground sm:grid-cols-2">
          ...
          <p>
            <strong className="text-foreground">Liquidar pendentes</strong> — após sync, partidas
            finalizadas Over 2.5 são fechadas automaticamente (FT + placar). Use Void manual em casos especiais.
          </p>
        </CardContent>
      </Card>
